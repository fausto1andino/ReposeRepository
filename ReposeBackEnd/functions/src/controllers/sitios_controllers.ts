...
export async function createsitios(req: Request, res: Response) {           
    try{                    
        ...
        return res.status(201).json(Respuesta('sitio agregado', `El sitio fue agregado correctamente con el id ${sitiosAdded.id}`, newsitios));
    }
    catch(err){
        return handleError(res, err);
    }
...
export async function consultarsitios(req: Request, res: Response) {           
    try{
        ...
        if(!doc) {
            return res.status(404).json(Respuesta('sitios no encontrado', `sitios con el id ${req.params.id} no encontrado`, {req}));               
        }        
